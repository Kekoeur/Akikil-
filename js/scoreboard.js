/**
 * Logique principale du jeu "Qui est-ce ?" avec scoreboard amélioré
 */
...
/**
 * Affiche le scoreboard dans un conteneur donné - VERSION AMÉLIORÉE
 */
function displayScoreboard(containerId) {
    ...
    scores.forEach((scoreData, index) => {
        ...
        const isCurrentPlayer = scoreData.playerName === currentPlayerName && 
                              Math.abs(scoreData.timestamp - new Date().getTime()) < 120000; // 2 minutes
        
        // Format de date amélioré - très petit
        const gameDate = new Date(scoreData.date);
        const dateStr = gameDate.toLocaleDateString('fr-FR', {
            day: '2-digit',
            month: '2-digit',
            year: '2-digit'
        });
        const timeStr = gameDate.toLocaleTimeString('fr-FR', {
            hour: '2-digit',
            minute: '2-digit'
        });
        ...
        html += `
            <div class="score-item ${getRankClass(rank)} ${isCurrentPlayer ? 'current-player' : ''}">
                ...
                <div class="score-details">
                    ...
                    <div class="score-date">${dateStr}<br><span style="font-size: 0.65rem; opacity: 0.7;">${timeStr}</span></div>
                </div>
            </div>
        `;
    });
...
/**
 * Formate une durée en secondes - VERSION AMÉLIORÉE
 */
...
/**
 * Vide le scoreboard avec confirmation par mot de passe - VERSION CORRIGÉE
 */
...
/**
 * Confirme le reset avec vérification du mot de passe - VERSION CORRIGÉE
 */
function confirmReset() {
    ...
    // Afficher le chargement
    confirmBtn.innerHTML = '<div class="loading"></div>VÉRIFICATION...';
    confirmBtn.disabled = true;
    
    // Mot de passe administrateur pour la suppression du scoreboard
    const ADMIN_PASSWORD = 'adminRESET2025';
    
    // Vérifier le mot de passe administrateur
    if (password !== ADMIN_PASSWORD) {
        showConfirmError('❌ Mot de passe administrateur incorrect !');
    ...
    // Mot de passe correct, procéder au reset immédiatement
    ...
    try {
        ...
        showSuccessMessage('✅ Scoreboard vidé avec succès !');
